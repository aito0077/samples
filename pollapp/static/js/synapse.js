...
(function(window) {
  ...
  Synapse.getEvents = function(subject, event) {
    var events;
    if (!event) {
    	//console.log('Subject type: '+subject.type + ' Synapse.types.jquery:  '  + Synapse.types.jquery  +  ' Synapse.types.model: ' + Synapse.types.model);
      if (subject.type === Synapse.types.jquery) {
        events = [detectDomEvent(subject)];
      } else if (subject.type === Synapse.types.model || subject.type===0) {
        events = ['change'];
      } else {
